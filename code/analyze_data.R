...
nhanes_processed <- read.csv("nhanes_processed.csv")
summary(nhanes_processed)

nhanes <- nhanes_processed %>%
  mutate(age = RIDAGEYR,
         sex = factor(RIAGENDR, levels = c(1, 2), labels = c("male", "female")),
         race = factor(RIDRETH3, levels = c(3, 4, 1, 2, 6, 7), 
                       labels = c("white", "black", "hispanic", "hispanic", 
                                  "asian", "other")),
         education = factor(DMDEDUC2, levels = c(1, 2, 3, 4, 5), 
                            labels = c("under 9th", "9-11th", "high school", 
                                       "college", "college graduate or above"), 
                            NA),
         income = factor(ifelse(INDFMIN2 == 1 | INDFMIN2 == 2 | INDFMIN2 == 3 | 
                                  INDFMIN2 == 4 | INDFMIN2 == 13, "under 20000", 
                                ifelse(INDFMIN2 == 5 | INDFMIN2 == 6 | 
                                         INDFMIN2 == 7 | INDFMIN2 == 12, 
                                       "20000-44999", 
                                       ifelse(INDFMIN2 == 8 | INDFMIN2 == 9 | 
                                                INDFMIN2 == 10, "45000-74999", 
                                              ifelse(INDFMIN2 == 14 | 
                                                       INDFMIN2 == 15, 
                                                     "75000 and above", NA))))),
         psu = SDMVPSU,
         ...
         caff_c = factor(ifelse(DR1TCAFF < 50, "low", 
                                ifelse(DR1TCAFF >= 50 & DR1TCAFF < 200, 
                                       "normal", 
                                       ifelse(DR1TCAFF >= 200 & DR1TCAFF < 400, 
                                              "high", 
                                              ifelse(DR1TCAFF >= 400, 
                                                     "very high", NA)))), 
         ...
         alcohol = factor(ifelse(DR1TALCO <= 14, "less than 1 drink", 
                                 ifelse(DR1TALCO > 14 & DR1TALCO <= 28, 
                                        "1-2 drinks", 
                                        ifelse(DR1TALCO > 28, 
                                               "more than 2 drinks", NA))), 
                          levels = c("less than 1 drink", "1-2 drinks", 
                                     "more than 2 drinks")),
         wt = BMXWT,
         bmi = BMXBMI,
         bmi_c = factor(ifelse(BMXBMI < 18.5, "Underweight", 
                               ifelse(BMXBMI >= 18.5 & BMXBMI < 25, "Normal", 
                                      ifelse(BMXBMI >= 25 & BMXBMI < 30, 
                                             "Overweight", 
                                             ifelse(BMXBMI >= 30, "Obese", 
                                                    NA)))),
         ...
         gallstone = factor(MCQ550, levels = c(0, 1, 2), labels = c(0, "Yes", 
                                                                    "No"), NA),
         gallsur = factor(MCQ560, levels = c(0, 1, 2), labels = c(0, "Yes", 
                                                                  "No"), NA),
         galldis = factor(ifelse(MCQ550 == 1 | MCQ560 == 1, "Yes", 
                                 ifelse((MCQ550 == 0 | MCQ550 == 2) & 
                                          MCQ560 == 2, "No", 
                                        ifelse((MCQ560 == 0 | MCQ560 == 2) & 
                                                 MCQ550 == 2, "No", NA))), 
                          levels = c("No", "Yes")),
         diabetes = factor(DIQ010, levels = c(1, 2), labels = c("Yes", "No"), 
                           NA),
         smoking = factor(ifelse(SMQ020 == 1 & (SMQ040 == 1 | SMQ040 == 2), 
                                 "Yes", 
                                 ifelse(SMQ020 == 2 | (SMQ040 == 3), "No", NA))),
         pregnant = factor(ifelse(RHD143 == 1 & RIAGENDR == 2, "Yes", 
                                  ifelse(RIAGENDR == 1 | RHD143 == 2, "No", NA))),
         lab_chol = LBXTC,
         PAQ605 = ifelse(is.na(PAQ605), 0, PAQ605),
         ...
         pa_work_t = factor(ifelse((PAD615 < 1000 & PAD615 >= 75) | 
                                     (PAD630 < 1000 & PAD630 >= 150), "AHA", 
                                   ifelse(PAD615 < 75 & PAD630 < 150, "non-AHA", 
                                          NA))),
         pa_work = factor(ifelse(pa_work_c == "Yes" & pa_work_t == "AHA", 
                                 "enough", 
                                 ifelse(pa_work_c == "Yes" & 
                                          pa_work_t == "non-AHA", "not enough", 
                                        ifelse(pa_work_c == "No" & 
                                                 pa_work_t == "non-AHA", 
                                               "don't exercise", NA)))),
         pa_act_c = factor(ifelse(PAQ650 == 1 | PAQ665 == 1, "Yes", 
                                  ifelse(PAQ650 == 2 & PAQ665 == 2, "No", NA))),
         pa_act_t = factor(ifelse((PAD660 < 1000 & PAD660 >= 75) | 
                                    (PAD675 < 1000 & PAD675 >= 150), "AHA", 
                                  ifelse(PAD660 < 75 & PAD675 < 150, "non-AHA", 
                                         NA))),
         pa_act = factor(ifelse(pa_act_c == "Yes" & pa_act_t == "AHA", "enough", 
                                ifelse(pa_act_c == "Yes" & pa_act_t == "non-AHA", 
                                       "not enough", 
                                       ifelse(pa_act_c == "No" & 
                                                pa_act_t == "non-AHA", 
                                              "don't exercise", NA)))),
         pa = factor(ifelse(pa_work == "enough" | pa_act == "enough", "enough", 
                            ifelse((pa_work == "not enough" | 
                                      pa_work == "don't exercise") & 
                                     pa_act == "not enough", "not enough", 
                                   ifelse((pa_act == "not enough" | 
                                             pa_act == "don't exercise") & 
                                            pa_work == "not enough", "not enough", 
                                          ifelse(pa_work == "don't exercise" & 
                                                   pa_act == "don't exercise", 
                                                 "not enough", NA)))),
...
nhanes_wt <- svydesign(id = ~psu, weights = ~weighted, strata = ~stra, 
                       data = nhanes_final, nest = T)
options(survey.lonely.psu = "adjust")
summary(nhanes_wt)

# Continuous variables, mean and SE
Table1.con <- svyby(~ age + wt + bmi + caff + fiber + fat + chol + vitc + water, 
                   ~ galldis + keep, nhanes_wt, svymean, ci = T)
Table1.con.tot <- as.data.frame(
  svymean(~ age + wt + bmi + caff + fiber + fat + chol + vitc + water, na.rm = T, 
          subset(nhanes_wt, keep == 1)))

# Categorical variables, unweighted count and percentage
# uwtd count
svyby(~ galldis, by = ~ galldis + sex, nhanes_wt, unwtd.count)
...
# Percentage
Table1.cat <- svyby(~ sex + race + education + caff_c + alcohol + bmi_c + 
                      smoking + pa, ~ galldis + keep, nhanes_wt, svymean)
Table1.cat.tot <- as.data.frame(
  svymean(~ sex + race + education + caff_c + alcohol + bmi_c + smoking + pa, 
          subset(nhanes_wt, keep == 1)) * 100)
...
dis_df <- dis_df %>%
  ...
  pivot_longer(names_to = "Levels", values_to = "values", 
               cols = galldisNo:galldisYes) 
...
ggplot(dis_df, aes(x = caff_c, y = values, fill = Levels, label = percent)) + 
  ...
  labs(x ="Caffeine Intake Categories", y = "Proportion of subjects", 
       fill = "Gallstones") +
  theme_apa() +
...
ttest.col <- c("age ~ galldis", "wt ~ galldis", "bmi ~ galldis", "caff ~ galldis", 
               "fiber ~ galldis", "fat ~ galldis", "chol ~ galldis", 
               "vitc ~ galldis", "water ~ galldis")
...
# Create a character vector with the combinations
chisq.col <- c("sex + galldis", "race + galldis", "education + galldis", 
               "caff_c + galldis", "alcohol + galldis", "bmi_c + galldis", 
               "smoking + galldis", "pa + galldis", "fat_c + galldis")
...
glm_full <- svyglm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + pa, 
                   subset(nhanes_wt, keep == 1), family = binomial()) 
summary(glm_full)
...
# Anova F-test
glm_pa <- svyglm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat, 
                 subset(nhanes_wt, keep == 1), family = binomial())
summary(glm_pa)
anova(glm_full, glm_pa, method = "LRT") # p = 0.51
...
# Create a character vector with the combinations, categorical variables only
regtest.col <- c("~caff_c", "~sex", "~race", "~pa")

# Anova F-test, looping
regtest_result <- list()
regtest_result<- lapply(regtest.col, regtest) # caff:0.936; sex:0.027; race:0.171; pa:0.512

## Adjusted model
# Select caff_c, sex, race, age, bmi, fiber, fat
glm_adj <- svyglm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat, 
                  subset(nhanes_wt, keep == 1), family = binomial())
summary(glm_adj)
summ(glm_adj)
...
regtest_result <- list()
regtest_result<- lapply(regtest.col, regtest) # caff_c:0.946; sex:0.007; race:0.111

## Assumption checking (Diagnostics)
# Collinearity, use unweighted sample
uwt_adj <- glm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat, 
               data = nhanes_c, family = "binomial")
summary(uwt_adj)
...
# Age
uwt_age <- glm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + agesq, 
               data = nhanes_c_lineary, family = "binomial")
summary(uwt_age) # age^2: p = 0.068
...
# BMI
uwt_bmi <- glm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + bmisq, 
               data = nhanes_c_lineary, family = "binomial")
summary(uwt_bmi) # bmi^2: p = 0.18
...
# Fiber
uwt_fiber <- glm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + 
                   fibersq, data = nhanes_c_lineary, family = "binomial")
summary(uwt_fiber) # fiber^2: p = 0.15
...
# Fat
uwt_fat <- glm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + fatsq, 
               data = nhanes_c_lineary, family = "binomial")
summary(uwt_fat) # fat^2: p = 0.85
...
#Leverage plot
ggplot(data = resid.adj, aes(x = .std.resid, y = h)) +
  geom_point() +
...
# DBeta
influence_df <- as.data.frame(influence.measures(glm_adj)$infmat)

dbeta_df <- data.frame(
  dDev = residuals(glm_adj, type = "deviance")^2
)
dbeta_df <- dbeta_df %>%
  cbind(nhanes_c) %>%
  arrange(SEQN)
...
ggplot(data = dbeta_df, aes(x = SEQN, y = dDev)) +
  geom_point() +
  theme_bw() +
...
outlier_rows <- which(grepl("Yes", dbeta_df$outlier)) # identify the row no.

nhanes_c <- nhanes_c %>% slice(-outlier_rows)

## Unadjusted model
glm_uni_c <- svyglm(galldis ~ caff_c, subset(nhanes_wt, keep == 1), 
...
f <- factor(c("Fat (5g)", "Fiber (5g)", "BMI", "Age (5y)", "Other races", 
              "Asian", "Hispanic", "Black", "Female", "Very high caffeine", 
...
plotdf %>%
  ...
  geom_vline(xintercept = 1, size = .3, linetype = "dashed") +
  geom_errorbarh(data = filter(plotdf, model == "unadjusted"), 
                 aes(xmax = ORH, xmin = ORL), 
                 size = .4, height = .2, 
                 color = "gray25", position = position_nudge(y = adj)) +
  geom_point(data = filter(plotdf, model == "unadjusted"), 
             aes(color = "Unadjusted model  p = 0.629\nPseudo R^2 = 0.001"), 
             shape = 21, size = 2, fill = "red", 
             position = position_nudge(y = adj)) +
  geom_errorbarh(data = filter(plotdf, model == "adjusted"), 
                 aes(xmax = ORH, xmin = ORL), 
                 size = .4, height = .2, color = "gray25") +
  geom_point(data = filter(plotdf, model == "adjusted"), 
             aes(color = "Adjusted model  p < 0.001\nPseudo R^2 = 0.101"),
             shape = 21, size = 2, fill = "lightgreen")+
  ...
  guides(colour = guide_legend(override.aes = list(fill = c("lightgreen","red"))),
...
glm_int_race <- svyglm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + 
                         caff_c*race, subset(nhanes_wt, keep == 1), 
                       family = quasibinomial())
summary(glm_int_race, df = degf(nhanes_wt)) 
regTermTest(glm_int_race, ~ caff_c*race, method = "LRT", df = degf(nhanes_wt)) # p = 0.14
# overall caff_c*race isn't significant

# Does this relationship differ between sex, due to hormone?
glm_int_sex <- svyglm(galldis ~ caff_c + sex + race + age + bmi + fiber + fat + 
                        caff_c*sex, subset(nhanes_wt, keep==1), 
                      family = quasibinomial())
...
## Compare model fit
regTermTest(glm_uni_c, ~ caff_c, method = "LRT") # p = 0.65
regTermTest(glm_adj, ~ caff_c + sex + race + age + bmi + fiber + fat, 
            method = "LRT", df = degf(nhanes_wt)) # p < 0.001
regTermTest(glm_adj, ~ sex + race + age + bmi + fiber + fat, method = "LRT", 
            df = degf(nhanes_wt)) # p < 0.001
regTermTest(glm_int_sex, ~ caff_c + sex + race + age + bmi + fiber + fat + 
              caff_c*sex, method = "LRT", df = degf(nhanes_wt)) # p < 0.001
regTermTest(glm_int_sex, ~ caff_c*sex, method = "LRT", df = degf(nhanes_wt)) # p = 0.014
...
psrsq(glm_int_sex, type="Nagelkerke") 
# model with the interaction term is the best one!
...
nhanes_sf_wt <- svydesign(id = ~psu, weights = ~weighted, strata = ~stra, 
                          data = nhanes_final_sf, nest = T)
options(survey.lonely.psu = "adjust")

# Adjusted model for men
glm_sf_male <- svyglm(galldis ~ caff_c + race + age + bmi + fiber + fat, 
                      subset(nhanes_sf_wt, sf == 1), family = quasibinomial)
summary(glm_sf_male)
...
regTermTest(glm_sf_male, ~caff_c, method = "LRT") # p = 0.758

# Adjusted model for women
glm_sf_female <- svyglm(galldis ~ caff_c + race + age + bmi + fiber + fat, 
                        subset(nhanes_sf_wt, sf == 2), family = quasibinomial)
...
regTermTest(glm_sf_female, ~caff_c, method = "LRT") # p = 0.972
...
adj <- .1
colors_sf <- c("Men" = "gray25", "Women" = "gray25")

sf_plot %>%
  ggplot(aes(x = OR, y = term)) +
  geom_vline(aes(xintercept = 1), size = .2, linetype = "dashed") +
  geom_errorbar(data = filter(sf_plot, model == "Men"), 
                aes(xmax = ORH, xmin = ORL), 
                size = .4, width = 0.2, color = "gray25", 
                position = position_nudge(y = adj)) +
  geom_point(data = filter(sf_plot, model == "Men"), aes(color = "Men"), 
             shape = 21, size = 2, fill = "lightblue", 
             position = position_nudge(y = adj)) +
  geom_errorbar(data = filter(sf_plot, model == "Women"), 
                aes(xmax = ORH, xmin = ORL), 
                size = .4, width = 0.2, color = "gray25") +
  geom_point(data = filter(sf_plot, model == "Women"), aes(color = "Women"),
             shape = 21, size = 2, fill = "pink") +
  theme_apa(remove.x.gridlines = F) +
  scale_y_discrete(limits = fct_inorder(caf)) +
  scale_colour_manual(values = colors_sf) + 
  guides(colour = guide_legend(override.aes = list(fill = c("lightblue", "pink"))),
         fill = guide_legend(ncol = 2, nrow = 2, byrow = T))+
  labs(y = "Caffeine Intake Categories", x = "Odds ratio") +
  theme(
    plot.title = element_text(size = 10, face = "bold"),
